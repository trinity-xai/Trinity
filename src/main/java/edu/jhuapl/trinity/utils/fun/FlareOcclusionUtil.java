...
public class FlareOcclusionUtil {
    ...
    /**
     * Computes a visibility factor [0.0, 1.0] for the sun based on GUI occlusion.
     * 1.0 = fully visible, 0.0 = fully occluded, values in-between indicate partial occlusion/falloff.
     *
     ...
     * @param fadeRadius        Number of pixels around the occluder edge to apply soft fade-out.
...
public static double computeSunOcclusionFactor(
    ...
    if (sunScenePos == null || occluders == null || occluders.isEmpty()) return 1.0;

    double worstOcclusion = 0.0;

    for (Node node : occluders) {
        if (!node.isVisible() || node.getOpacity() <= 0.0) continue;
...
private static double smoothstep(double t) {
    ...
    /**
     * Recursively collects all visible, non-transparent nodes inside a container that could occlude the flare.
     *
    ...
    private static void collectOccludersRecursive(Parent parent, List<Node> result) {
        for (Node node : parent.getChildrenUnmodifiable()) {
            ...
            if (ignore instanceof Boolean && (Boolean) ignore) continue;

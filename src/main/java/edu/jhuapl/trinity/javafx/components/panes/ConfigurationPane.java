 ...
 */
public class ConfigurationPane extends PathPane {
    Scene scene;
    ...
    private static BorderPane createContent() {
        ...
        FXMLLoader loader = new FXMLLoader(ConfigurationPane.class.getResource("/edu/jhuapl/trinity/fxml/Configuration.fxml"));
        loader.setLocation(ConfigurationPane.class.getResource("/edu/jhuapl/trinity/fxml/Configuration.fxml"));
        BorderPane sgRoot;
        AnchorPane configurationAnchorPane;
        try {
            configurationAnchorPane = loader.load();
            sgRoot = new BorderPane(configurationAnchorPane);
            sgRoot.setBackground(transBack);
        } catch (IOException ex) {
            sgRoot = new BorderPane(new Text("Unable to load Configuration view."));
            Logger.getLogger(ConfigurationPane.class.getName()).log(Level.SEVERE, null, ex);
        }
        return sgRoot;
    }

    public ConfigurationPane(Scene scene, Pane parent) {
        super(scene, parent, 500, 600, createContent(), "Configuration ", "", 200.0, 300.0);
        this.scene = scene;
